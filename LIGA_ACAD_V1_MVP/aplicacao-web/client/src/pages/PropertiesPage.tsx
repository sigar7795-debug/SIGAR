...
import { ArrowRight, Check, LandPlot, Landmark, Link2, Mail, MapPin, Pencil, Plus, Search, SearchX, ShieldCheck, Sprout, Trash2, UserCog, UserPlus, UsersRound, X } from "lucide-react";
...
type MemberRole = "proprietario" | "editor" | "visualizador";
type MemberStatus = "pendente" | "ativo" | "revogado" | "recusado";
type PropertyMember = { id: number; role: MemberRole; status: MemberStatus; invitedEmail: string; userId: number | null; name: string | null };
...
const memberRoleLabels: Record<MemberRole, string> = { proprietario: "Proprietário", editor: "Editor", visualizador: "Visualizador" };
const memberStatusLabels: Record<MemberStatus, string> = { pendente: "Pendente", ativo: "Ativo", revogado: "Revogado", recusado: "Recusado" };
...
export default function PropertiesPage() {
  ...
  const pendingInvitesQuery = trpc.finance.members.pendingInvites.useQuery();
  ...
  const [membersProperty, setMembersProperty] = useState<{ id: number; name: string; canManage: boolean } | null>(null);
  const [editingPropertyId, setEditingPropertyId] = useState<number | null>(null);
  const [form, setForm] = useState<PropertyForm>(initialForm);
  const [editForm, setEditForm] = useState<PropertyForm>(initialForm);
  ...
  const acceptInvite = trpc.finance.members.acceptInvite.useMutation({
    onSuccess: () => {
      void utils.finance.members.pendingInvites.invalidate();
      refreshProperties();
      toast.success("Convite aceite. A propriedade já está disponível para si.");
    },
    onError: error => toast.error(error.message),
  });
  const declineInvite = trpc.finance.members.declineInvite.useMutation({
    onSuccess: () => {
      void utils.finance.members.pendingInvites.invalidate();
  ...
  const updateProperty = trpc.finance.properties.update.useMutation({
    onSuccess: () => {
      refreshProperties();
      setEditingPropertyId(null);
      toast.success("Dados da propriedade atualizados com sucesso.");
    },
    onError: error => toast.error(error.message),
  });
  ...
  const openEditDialog = (property: { id: number; name: string; municipality: string | null; state: string | null; totalArea: string | null; mainActivity: string | null; description: string | null }) => {
    setEditForm({
      name: property.name,
      municipality: property.municipality ?? "",
      state: property.state ?? "",
      totalArea: property.totalArea ?? "",
      mainActivity: property.mainActivity ?? "",
      description: property.description ?? "",
      userCpfs: [],
    });
    setEditingPropertyId(property.id);
  };
  const submitEditProperty = (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    ...
    updateProperty.mutate({
      ...
      totalArea: editForm.totalArea ? Number(editForm.totalArea.replace(",", ".")) : undefined,
      mainActivity: editForm.mainActivity || undefined,
  ...
  return <div className="space-y-7 sm:space-y-9"><PageHeader eyebrow="Base da operação" title="Propriedades" description="Gerencie os espaços que fazem parte da sua operação." action={actions} />
    <PendingInvitesBanner invites={pendingInvitesQuery.data ?? []} onAccept={id => acceptInvite.mutate({ memberId: id })} onDecline={id => declineInvite.mutate({ memberId: id })} loading={acceptInvite.isPending || declineInvite.isPending} />
    ...
    {!hasDomainUsers ? <EmptyState icon={UsersRound} title="Cadastre o primeiro utilizador proprietário" description="Antes de criar uma propriedade, informe ao menos um titular com nome, sexo e CPF válido. O CPF é exibido mascarado nas listas." action={<Button onClick={() => setDomainUserOpen(true)} className="rounded-none bg-primary font-bold text-primary-foreground hover:bg-primary/90"><UserPlus className="mr-2 h-4 w-4" /> Cadastrar utilizador</Button>} /> : !propertiesQuery.data?.length ? <EmptyState icon={Landmark} title="Nenhuma propriedade cadastrada" description="Selecione um ou mais utilizadores proprietários ao criar a primeira propriedade. Outros coproprietários poderão ser vinculados depois." action={<Button onClick={() => setPropertyOpen(true)} className="rounded-none bg-primary font-bold text-primary-foreground hover:bg-primary/90"><Plus className="mr-2 h-4 w-4" /> Cadastrar propriedade</Button>} /> : !filteredProperties.length ? <EmptyState icon={SearchX} title="Nenhuma propriedade encontrada" description="Ajuste a busca ou o filtro de status para visualizar outros resultados." action={<Button variant="outline" onClick={() => { setSearch(""); setStatusFilter("all"); }} className="rounded-none">Limpar filtros</Button>} /> : <section className="grid gap-0 border-y border-olive/35 md:grid-cols-2 xl:grid-cols-3">{filteredProperties.map(property => {
      ...
      return <article key={property.id} className={`group relative overflow-hidden border-b border-r border-olive/30 bg-card p-5 transition-colors ${selected ? "bg-accent" : "hover:bg-accent/45"}`}><div className="relative flex items-start justify-between gap-4"><div className="grid h-11 w-11 place-items-center border border-olive/30 bg-accent text-accent-foreground"><LandPlot className="h-5 w-5" /></div><Badge className={`rounded-none border ${selected ? "border-field bg-field text-sand" : "border-positive/25 bg-positive/10 text-positive"}`}>{selected ? "Selecionada" : "Ativa"}</Badge></div><div className="relative mt-6"><h2 className="font-display text-2xl font-bold text-foreground">{property.name}</h2><p className="mt-2 flex min-h-5 items-center text-sm text-muted-foreground"><MapPin className="mr-1.5 h-3.5 w-3.5 text-muted-foreground" />{property.municipality ? `${property.municipality}${property.state ? `, ${property.state}` : ""}` : "Localização não informada"}</p><div className="mt-6 grid gap-3 border-t border-border pt-4 text-sm"><PropertyMeta label="Atividade" value={property.mainActivity || "Não informada"} /><PropertyMeta label="Área" value={property.totalArea ? `${Number(property.totalArea).toLocaleString("pt-BR")} ha` : "Não informada"} /><PropertyMeta label="Atualização" value={formatDate(property.updatedAt)} /></div><div className="mt-5 border-t border-border pt-4"><div className="flex items-center justify-between gap-3"><p className="font-mono text-[9px] font-semibold uppercase tracking-[0.14em] text-olive">Proprietários (CPF)</p><Button variant="ghost" size="sm" onClick={() => setLinkProperty({ id: property.id, name: property.name, domainUsers: property.domainUsers })} className="h-7 rounded-none px-2 text-xs text-primary hover:bg-accent hover:text-accent-foreground"><Link2 className="mr-1.5 h-3.5 w-3.5" /> Vincular</Button></div><div className="mt-2 flex flex-wrap gap-1.5">{property.domainUsers.length ? property.domainUsers.map(domainUser => <Badge key={domainUser.cpf} className="max-w-full rounded-none border border-border bg-muted font-medium text-muted-foreground hover:bg-muted" title={domainUser.name}>{domainUser.name} · {maskCpfForList(domainUser.cpf)}</Badge>) : <span className="text-xs text-muted-foreground">Sem titular vinculado</span>}</div></div><div className="mt-4 flex items-center justify-between gap-3 border-t border-border pt-4"><p className="font-mono text-[9px] font-semibold uppercase tracking-[0.14em] text-olive">Membros e permissões</p><Badge className="rounded-none border border-border bg-muted font-medium text-muted-foreground">{memberRoleLabels[property.effectiveRole]}</Badge></div><Button variant="ghost" size="sm" onClick={() => setMembersProperty({ id: property.id, name: property.name, canManage: property.effectiveRole === "proprietario" })} className="mt-2 h-7 rounded-none px-2 text-xs text-primary hover:bg-accent hover:text-accent-foreground"><UserCog className="mr-1.5 h-3.5 w-3.5" /> {property.effectiveRole === "proprietario" ? "Gerir membros" : "Ver membros"}</Button><Button onClick={() => { setPropertyId(property.id); setLocation("/dashboard"); }} variant="outline" className="mt-6 w-full rounded-none border-border bg-transparent text-foreground hover:bg-accent hover:text-accent-foreground">Ver detalhes <ArrowRight className="ml-2 h-4 w-4" /></Button>{property.effectiveRole === "proprietario" ? <Button onClick={() => openEditDialog(property)} variant="outline" className="mt-3 w-full rounded-none border-border bg-transparent text-foreground hover:bg-accent hover:text-accent-foreground"><Pencil className="mr-2 h-4 w-4" /> Editar dados</Button> : null}{canRemoveProperty ? <RemovePropertyDialog propertyName={property.name} onConfirm={() => deactivateProperty.mutate({ propertyId: property.id })} loading={deactivateProperty.isPending} /> : null}</div></article>;
    ...
    <PropertyDialog mode="edit" open={editingPropertyId !== null} onOpenChange={open => { if (!open) setEditingPropertyId(null); }} form={editForm} setForm={setEditForm} onSubmit={submitEditProperty} loading={updateProperty.isPending} users={domainUsers} />
    ...
    <MembersDialog property={membersProperty} onClose={() => setMembersProperty(null)} />
  </div>;
}
...
function PropertyDialog({ open, onOpenChange, form, setForm, onSubmit, loading, users, mode = "create" }: { open: boolean; onOpenChange: (open: boolean) => void; form: PropertyForm; setForm: (form: PropertyForm) => void; onSubmit: (event: FormEvent<HTMLFormElement>) => void; loading: boolean; users: DomainUser[]; mode?: "create" | "edit" }) {
...
function PendingInvitesBanner({ invites, onAccept, onDecline, loading }: { invites: { id: number; propertyId: number; propertyName: string; role: MemberRole }[]; onAccept: (id: number) => void; onDecline: (id: number) => void; loading: boolean }) {
  if (!invites.length) return null;
  return <section aria-label="Convites pendentes" className="space-y-2 border border-primary/30 bg-primary/5 p-4">
    <p className="font-mono text-[9px] font-semibold uppercase tracking-[0.18em] text-primary">Convites pendentes</p>
    <ul className="space-y-2">{invites.map(invite => <li key={invite.id} className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-border bg-card px-4 py-3"><span className="text-sm text-foreground">Foi convidado(a) para <strong>{invite.propertyName}</strong> como <strong>{memberRoleLabels[invite.role]}</strong>.</span><span className="flex gap-2"><Button size="sm" disabled={loading} onClick={() => onAccept(invite.id)} className="h-8 rounded-none bg-primary px-3 text-xs font-bold text-primary-foreground hover:bg-primary/90"><Check className="mr-1.5 h-3.5 w-3.5" /> Aceitar</Button><Button size="sm" variant="outline" disabled={loading} onClick={() => onDecline(invite.id)} className="h-8 rounded-none border-border px-3 text-xs text-muted-foreground hover:bg-accent"><X className="mr-1.5 h-3.5 w-3.5" /> Recusar</Button></span></li>)}</ul>
  </section>;
}

function MembersDialog({ property, onClose }: { property: { id: number; name: string; canManage: boolean } | null; onClose: () => void }) {
  const utils = trpc.useUtils();
  const membersQuery = trpc.finance.members.list.useQuery({ propertyId: property?.id ?? 0 }, { enabled: Boolean(property) });
  const [email, setEmail] = useState("");
  const [role, setRole] = useState<MemberRole>("visualizador");
  const invalidate = () => { if (property) void utils.finance.members.list.invalidate({ propertyId: property.id }); };
  const invite = trpc.finance.members.invite.useMutation({
    onSuccess: () => { invalidate(); setEmail(""); setRole("visualizador"); toast.success("Convite enviado."); },
    onError: error => toast.error(error.message),
  });
  const updateRole = trpc.finance.members.updateRole.useMutation({
    onSuccess: () => { invalidate(); toast.success("Papel atualizado."); },
    onError: error => toast.error(error.message),
  });
  const revoke = trpc.finance.members.revoke.useMutation({
    onSuccess: () => { invalidate(); toast.success("Acesso revogado."); },
    onError: error => toast.error(error.message),
  });
  const close = () => { setEmail(""); setRole("visualizador"); onClose(); };
  const submitInvite = (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    if (!property || !email.trim()) return;
    invite.mutate({ propertyId: property.id, email: email.trim(), role });
  };
  const members: PropertyMember[] = membersQuery.data ?? [];

  return <Dialog open={Boolean(property)} onOpenChange={next => { if (!next) close(); }}><DialogContent className="max-h-[90vh] overflow-y-auto border-border bg-card p-0 text-card-foreground sm:max-w-lg"><DialogHeader className="border-b border-border px-6 py-6"><div className="mb-3 grid h-10 w-10 place-items-center rounded-xl bg-accent text-accent-foreground"><ShieldCheck className="h-5 w-5" /></div><DialogTitle className="font-display text-2xl font-extrabold tracking-[-0.04em] text-foreground">Membros de {property?.name}</DialogTitle><DialogDescription className="leading-6 text-muted-foreground">Contas com acesso a esta propriedade, o papel de cada uma e o estado do convite.</DialogDescription></DialogHeader><div className="space-y-5 px-6 py-6">
    {membersQuery.isLoading ? <p className="text-sm text-muted-foreground">A carregar membros...</p> : members.length ? <ul className="space-y-2">{members.map(member => <li key={member.id} className="flex items-center justify-between gap-3 rounded-xl border border-border bg-background px-4 py-3"><span className="min-w-0"><span className="block truncate font-semibold text-foreground">{member.name || member.invitedEmail}</span><span className="mt-0.5 block text-xs text-muted-foreground">{member.invitedEmail}</span></span><span className="flex shrink-0 items-center gap-2"><Badge className="rounded-none border border-border bg-muted font-medium text-muted-foreground">{memberStatusLabels[member.status]}</Badge>{property?.canManage ? <Select value={member.role} onValueChange={value => property && updateRole.mutate({ propertyId: property.id, memberId: member.id, role: value as MemberRole })}><SelectTrigger className="h-8 w-36 rounded-none border-border bg-transparent text-xs"><SelectValue /></SelectTrigger><SelectContent className="rounded-none"><SelectItem value="proprietario">Proprietário</SelectItem><SelectItem value="editor">Editor</SelectItem><SelectItem value="visualizador">Visualizador</SelectItem></SelectContent></Select> : <Badge className="rounded-none border border-border bg-muted font-medium text-muted-foreground">{memberRoleLabels[member.role]}</Badge>}{property?.canManage && member.status !== "revogado" ? <Button variant="ghost" size="sm" onClick={() => property && revoke.mutate({ propertyId: property.id, memberId: member.id })} className="h-8 rounded-none px-2 text-xs text-destructive hover:bg-destructive/10 hover:text-destructive"><X className="h-3.5 w-3.5" /></Button> : null}</span></li>)}</ul> : <p className="rounded-xl border border-dashed border-border p-4 text-sm leading-6 text-muted-foreground">Ainda não há membros convidados para esta propriedade.</p>}
    {property?.canManage ? <form onSubmit={submitInvite} className="space-y-3 border-t border-border pt-5"><Field label="Convidar por e-mail" required><div className="flex gap-2"><Input required type="email" value={email} onChange={event => setEmail(event.target.value)} placeholder="conta@exemplo.com" className="h-10 border-border bg-background text-foreground placeholder:text-muted-foreground focus-visible:ring-primary" /><Select value={role} onValueChange={value => setRole(value as MemberRole)}><SelectTrigger className="h-10 w-40 border-border bg-background"><SelectValue /></SelectTrigger><SelectContent className="border-border bg-card text-card-foreground"><SelectItem value="proprietario">Proprietário</SelectItem><SelectItem value="editor">Editor</SelectItem><SelectItem value="visualizador">Visualizador</SelectItem></SelectContent></Select></div></Field><Button type="submit" disabled={invite.isPending || !email.trim()} className="w-full bg-primary font-bold text-primary-foreground hover:bg-primary/90"><Mail className="mr-2 h-4 w-4" /> {invite.isPending ? "A convidar..." : "Enviar convite"}</Button></form> : null}
  </div><DialogFooter className="border-t border-border px-6 py-5"><Button variant="outline" onClick={close} className="border-border bg-transparent text-muted-foreground hover:bg-accent hover:text-accent-foreground">Fechar</Button></DialogFooter></DialogContent></Dialog>;
}
